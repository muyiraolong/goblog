...
func readTemplate(templates []string, templateDir string) ([]TemplateBlog, error) {
	...
	for _, view := range templates {
		...
		t.Funcs(template.FuncMap{"isODD": IsODD, "getNextName": GetNextName, "date": Date, "dateDay": DateDay}) //给html判断方法一个函数
		var err error
		t, err = t.ParseFiles(templateDir+viewName, home, header, footer, personal, post, pagination) //解析模板
		if err != nil {
			log.Println("解析模板有问题", err)
			return nil, err
		}
		var tb TemplateBlog
		tb.Template = t
