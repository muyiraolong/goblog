...
var NewContextFunc = NewContext()

type Context struct {
	Request  *http.Request
	W        http.ResponseWriter
	routers  map[string]func(ctx *Context)
	pathArgs map[string]map[string]string
}

func NewContext() *Context {
	ctx := &Context{}
	ctx.routers = make(map[string]func(ctx2 *Context))
...
func (ctx *Context) Handler(url string, f func(context *Context)) {
	UrlTree.Insert(url)
	ctx.routers[url] = f
}
...
func (ctx *Context) GetForm(key string) (string, error) {
...
func (ctx *Context) GetJson(key string) interface{} {
